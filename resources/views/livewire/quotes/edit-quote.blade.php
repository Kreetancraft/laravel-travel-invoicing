<div class="space-y-6 max-w-4xl">
    <div>
        <h1 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">{{ __('Edit Quote / Proposal') }}</h1>
        <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('Update itemized pricing, discounts, deposit terms, and client details.') }}</p>
    </div>
    ...
    <form wire:submit.prevent="save" class="space-y-6">
        ...
        <!-- Actions -->
        <div class="flex items-center justify-end gap-3">
            <flux:button href="{{ route('admin.quotes.show', $quoteId) }}" variant="ghost" wire:navigate>{{ __('Cancel') }}</flux:button>
            <flux:button type="submit" variant="primary" icon="check">{{ __('Update Quote Proposal') }}</flux:button>
        </div>
    </form>
</div>
